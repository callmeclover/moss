...
fn main() {
    ...
    let kernel_path: String = env::var("CARGO_BIN_FILE_KERNEL").unwrap();
    let disk_builder: DiskImageBuilder = DiskImageBuilder::new(PathBuf::from(kernel_path));

    // specify output paths
    let out_dir: PathBuf = PathBuf::from(env::var("OUT_DIR").unwrap());
    let uefi_path: PathBuf = out_dir.join("blog_os-uefi.img");
    let bios_path: PathBuf = out_dir.join("blog_os-bios.img");
